...
rustPlatform.buildRustPackage {
  name = "geobacter-cargo-${rustc.version}";
  inherit (rustc) version src cargoDeps;

  cargoSha256 = vendorSha256;
  postUnpack = ''
    (mkdir source/vendor && cd source/vendor && tar xf "${rustc.cargoDeps}" --strip-components=1);
  '';
  ...
  meta = with lib; {
    ...
    longDescription = ''
      Exactly like upstream Cargo, built with Geobacter.
    '';
